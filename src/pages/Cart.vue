      ...
      <template v-if="cart_data.length">
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div
            ...
            :key="item._key"
            class="flex flex-col md:flex-row md:items-center md:justify-between justify-center items-center py-4 border-b last:border-b-0 gap-4"
          >
            <!-- الصورة + تفاصيل الخدمة -->
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="text-primary font-bold">
                  {{ item.total_with_option.toFixed(2) }} د.أ
                </div>
              </div>
            </div>
            ...
            <!-- الكمية + الأكشنز -->
            <div
              class="flex flex-wrap md:flex-nowrap items-center gap-3 md:gap-4 justify-center"
            >
              <!-- التحكم في الكمية -->
              ...
              <div class="flex items-center gap-2 w-full md:w-auto">
                <button
                  ...
                  class="text-primary hover:text-white text-sm flex items-center gap-1 px-3 py-1 rounded border border-primary hover:bg-primary transition-colors w-full md:w-auto justify-center"
                >
                  <svg
                    class="w-4 h-4"
                    fill="none"
                ...
                <button
                  ...
                  class="text-red-500 hover:text-red-700 w-full md:w-auto flex justify-center"
                >
                  <svg
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          <div class="flex justify-between items-center mb-4">
            ...
            <span class="text-2xl font-bold text-primary"
              >{{ totalPrice.toFixed(2) }} د.أ</span
            >
          </div>
          <button
            class="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-primary-dark transition text-lg"
          >
            <router-link
              @click="storeCart"
              to="/checkout"
  ...
  try {
    if (newCount <= 0) {
      ...
      await api.post("/api/update-to-cart", {
        ...
        count: newCount,
        option_ids:
          JSON.stringify(item.service_options?.map((o) => o.id)) || [], // إرسال نفس الإضافات
      });
